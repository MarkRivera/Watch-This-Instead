...
const getMovies = async (req, res, next) => {
  ...
  if (!req.token) {
    ...
    let movies = await threeMovies;

    movies = movies.map(async movie => {
      const url = `https://api.themoviedb.org/3/movie/${movie.tmdbId}/images?api_key=${process.env.API_KEY}`;

      const data = await axios.get(url);
      // console.log(data.data.posters[0].file_path);
    });
    // Add three movies to req:
    req.movies = await threeMovies;
    next();
  } else {
    let randomPages = [0, 0, 0];
